...
#define RVBAP_VERSION \
  "rvbap v1.2.0 - (c) Olaf Matthes 2003-2007, based on vbap by Ville Pulkki"
...
// FIXME: should this be _WIN32 or MSVC?
#ifdef _WINDOWS
#define sqrtf sqrt
#endif
...
typedef struct vbap
{
  ...
  void *x_outlet3;
  void *x_outlet4;
  t_float x_set_inv_matx[MAX_LS_SETS][9]; //< inverse matrice for each ls set
  t_float x_set_matx[MAX_LS_SETS][9];     //< matrice for each loudspeaker set
  long x_lsset[MAX_LS_SETS][3]; //< loudspeaker channel numbers in each ls set
  ...
  t_float x_reverb_gs[MAX_LS_SETS]; //< ls set gain correct for equal volume
} t_rvbap;
#endif
...
// function prototypes
static void new_spread_dir(t_rvbap *x,
  t_float spreaddir[3], t_float vscartdir[3], t_float spread_base[3]);
static void new_spread_base(t_rvbap *x,
  t_float spreaddir[3], t_float vscartdir[3]);
#ifdef MAXMSP
...
static void *rvbap_new(t_symbol *s, int ac, t_atom *av); // using A_GIMME
...
{
  rvbap_class = class_new(gensym("rvbap"), (t_newmethod)rvbap_new, 0,
    (short)sizeof(t_rvbap), 0, A_GIMME, 0);

  class_addbang(rvbap_class, rvbap_bang);
  class_addmethod(rvbap_class, (t_method)rvbap_matrix,
...
int main(void)
{
  setup((t_messlist **)&rvbap_class, (method)rvbap_new, 0L,
    (short)sizeof(t_rvbap), 0L, A_GIMME, 0);

  addmess((method)rvbap_assist, "assist", A_CANT, 0);
  addbang((method)rvbap_bang);
  addinx((method)rvbap_in1, 1);
  addinx((method)rvbap_in2, 2);
  addinx((method)rvbap_in3, 3);
  addinx((method)rvbap_in4, 4);
  addftx((method)rvbap_ft1, 1);
  addftx((method)rvbap_ft2, 2);
...
static t_float atorad = (2.0 * M_PI / 360.0);
...
// converts cartesian coordinates to angular (polar)
static void cart_to_angle(t_float cvec[3], t_float avec[3])
{
...
static void vbap(t_float g[3], long ls[3], t_rvbap *x)
{
  ...
  // go through all defined loudspeaker sets and find the set which has all
  ...
  // If chosen set produced a negative value, make it zero and calculate
  // direction that corresponds  to these new gain values. This happens when the
  // virtual source is outside of all loudspeaker sets.
  if (dim == 3)
  {
    gains_modified = 0;
    for (i = 0; i < dim; i++)
...
// calculates gains to be added to previous gains, used in multiple direction
// panning (source spreading)
static void additive_vbap(t_float *final_gs, t_float cartdir[3], t_rvbap *x)
{
  t_float power;
  int i, j, k, gains_modified;
  ...
  if (gains_modified != 1)
  {
    ...
    final_gs[ls[0]-1] += g[0];
    final_gs[ls[1]-1] += g[1];
    // BUG FIX: this was causing negative indices with 2 dimensions so I made it
    // only try when using 3 dimensions. 2006-08-13 <hans@at.or.at>
    if (dim == 3)
      final_gs[ls[2]-1] += g[2];
  }
}
  ...
  t_float spreaddir[3], t_float vscartdir[3], t_float spread_base[3])
{
  ...
  power = sqrt(spreaddir[0]*spreaddir[0] +
               spreaddir[1]*spreaddir[1] +
...
// that causes some spreading. See theory in paper V. Pulkki
// "Uniform spreading of amplitude panned virtual sources" in WASPAA 99
static void spread_it(t_rvbap *x, t_float *final_gs)
{
  t_float vscartdir[3];
  ...
  if (x->x_spread > 70)
  {
    ...
    {
      final_gs[i] += (x->x_spread - 70) / 30.0 *
                     (x->x_spread - 70) / 30.0 * 10.0;
    }
  }
...
// calculate constant reverb gains for equally distributed reverb levels,
// this is achieved by calculating gains for a sound source that is everywhere,
// i.e. present in all directions
static void equal_reverb(t_rvbap *x, t_float *final_gs)
{
  t_float spreaddir[16][3];
  long i, spreaddirnum;
...
#ifdef MAXMSP
      // direct signals come first in matrix~
      SETLONG(&at[0], i+x->x_ls_amount);
      SETFLOAT(&at[1], (((oversqrtdist / x->x_dist) * x->x_reverb_gs[i]) +
                         (oversqrtdist * (1 - overdist) * final_gs[i])));
      outlet_list(x->x_outlet0, NULL, 2, at);
#endif
#ifdef PD
      // direct signals come first in matrix~
      SETFLOAT(&at[0], (i+x->x_ls_amount));
      SETFLOAT(&at[1], (((oversqrtdist / x->x_dist) * x->x_reverb_gs[i]) +
                         (oversqrtdist * (1 - overdist) * final_gs[i])));
      outlet_list(x->x_outlet0, gensym("list"), 2, at);
#endif
...
// read in loudspeaker matrices and calculate the gains for the equally
// distributed reverb signal part (i.e. global reverb)
static void rvbap_matrix(t_rvbap *x, t_symbol *s, int ac, t_atom *av)
{
  long counter = 0;
  long datapointer = 0;
  ...
  if (ac > 0)
  {
    ...
    {
      pd_error(x, "rvbap: dimension NaN");
  ...
  if (ac > 1)
  {
    ...
    {
      pd_error(x, "rvbap: loudspeaker set amount NaN");
      x->x_lsset_available = 0;
      return;
    }
  }
  ...
  if (counter <= 0)
  {
    pd_error(x, "rvbap %s: loudspeaker definitions exceed maximum number "
      "of speakers", s->s_name);
    x->x_lsset_available = 0;
    return;
  }
  ...
  while (counter-- > 0)
  {
    for (i = 0; i < x->x_dimension; i++)
    {
      ...
      {
        pd_error(x, "rvbap %s: param %ld is not an int", s->s_name, datapointer);
        x->x_lsset_available = 0;
        return;
      }
    }
    for (i = 0; i < x->x_dimension * x->x_dimension; i++)
    {
      ...
      {
        pd_error(x, "rvbap %s: param %ld is not a float",
          s->s_name, datapointer);
        x->x_lsset_available = 0;
        return;
      }
    }
  ...
  //}
#ifdef PD
  logpost(NULL, 3, "rvbap: loudspeaker setup configured");
#else
...
static void rvbap_in1(t_rvbap *x, long n) {x->x_azi = n;}

// panning angle elevation
static void rvbap_in2(t_rvbap *x, long n) {x->x_ele = n;}
...
// panning angle azimuth
static void rvbap_ft1(t_rvbap *x, double n) {x->x_azi = (long) n;}

// panning angle elevation
static void rvbap_ft2(t_rvbap *x, double n) {x->x_ele = (long) n;}
...
// MUST send it an int even if you do nothing with this int!!

...
{
  def_ls_class = class_new(gensym("define_loudspeakers"),
    (t_newmethod)def_ls_new, 0, (short)sizeof(t_def_ls), 0, A_GIMME, 0);

  class_addbang(def_ls_class, (t_method)def_ls_bang);
...
void main(void)
{
  setup((t_messlist **)&def_ls_class, (method)def_ls_new, 0L,
    (short)sizeof(t_def_ls), 0L, A_GIMME, 0);

  addbang((method)def_ls_bang);
...
static void def_ls_bang(t_def_ls *x)
{
  ...
  error("dimension == 3 for 3-D ls arrays "
        "(speakers also upward and/or downward");
}
...
static void def_ls_read_triplets(t_def_ls *x, t_symbol *s, int ac, Atom *av)
{
  ...
  {
    pd_error(x,
      "define_loudspeakers: can't specify loudspeaker triplets in 2-D setup");
    return;
  }
...
void vbap_def_ls(t_def_ls *x, t_symbol *s, int ac, Atom *av)
{
  ...
  // initialize object internal data from a ls-directions list
  initContent_ls_directions(x, ac, av);
...
// Selects the loudspeaker triplets, and calculates the inversion matrices for
// each selected triplet. A line (connection) is drawn between each loudspeaker.
// The lines denote the sides of the triangles. The triangles should not be
// intersecting. All crossing connections are searched and the  longer
// connection is erased. This yields non-intesecting triangles, which can be
// used in panning.
...
// International Conference on Auditory Displays 1998.
  ...
  for (i = 0; i < ls_amount; i++)
  {
    for (j = i+1; j < ls_amount; j++)
    {
      if (connections[i][j] == 1)
      {
        ...
        k = 0;
        while (distance_table[k] < distance) k++;
...
int any_ls_inside_triplet(int a, int b, int c,
  t_ls lss[MAX_LS_AMOUNT], int ls_amount)
{
  t_float invdet;
  t_ls *lp1, *lp2, *lp3;
  t_float invmx[9];
...
// calculate volume of the parallelepiped defined by the loudspeaker direction
// vectors and divide it with total length of the triangle sides. This is used
// when removing too narrow triangles.
t_float vol_p_side_lgth(int i, int j, int k, t_ls lss[MAX_LS_AMOUNT])
{
  ...
  if (lgth > 0.00001) // FIXME: should this be FLT_EPSILON?
    return volper / lgth;
  else
    return 0.0;
}
...
static int lines_intersect(int i,int j, int k, int l, t_ls lss[MAX_LS_AMOUNT])
{
  ...
  if (fabsf(dist_iv3)  <= FLT_EPSILON || fabsf(dist_jv3)  <= FLT_EPSILON ||
      fabsf(dist_kv3)  <= FLT_EPSILON || fabsf(dist_lv3)  <= FLT_EPSILON ||
      fabsf(dist_inv3) <= FLT_EPSILON || fabsf(dist_jnv3) <= FLT_EPSILON ||
      fabsf(dist_knv3) <= FLT_EPSILON || fabsf(dist_lnv3) <= FLT_EPSILON)
    return 0;

  // if crossing point is on line between both loudspeakers, return 1
  if (((fabsf(dist_ij - (dist_iv3 + dist_jv3))   <= FLT_EPSILON)  &&
       (fabsf(dist_kl - (dist_kv3 + dist_lv3))   <= FLT_EPSILON)) ||
      ((fabsf(dist_ij - (dist_inv3 + dist_jnv3)) <= FLT_EPSILON)  &&
       (fabsf(dist_kl - (dist_knv3 + dist_lnv3)) <= FLT_EPSILON)))
    return 1;
  else
    return 0;
}
...
// selects the loudspeaker pairs, calculates the inversion matrices and stores
// the data to a global array
static void choose_ls_tuplets(t_def_ls *x)
{
  ...
  {
    if (lss[sorted_lss[i+1]].azi - lss[sorted_lss[i]].azi <= 170)
    {
...
void sort_2D_lss(t_ls lss[MAX_LS_AMOUNT],

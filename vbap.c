...
// indicate that we are within VBAP object,
// specifically to include define_loudspeakers content within vbap
#define VBAP_OBJECT

#include "vbap.h"

// function prototypes
static void new_spread_dir(t_vbap *x,
  t_float spreaddir[3], t_float vscartdir[3], t_float spread_base[3]);
static void new_spread_base(t_vbap *x,
...
void vbap_assist(t_vbap *x, void *b, long m, long a, char *s)
{
  ...
  if (m == ASSIST_INLET)
  {
    ...
    {
      case 0:
        mess = "bang to calc and output vbap gains, loudspeakers definition";
        break;
...
void vbap_setup(void)
{
  vbap_class = class_new(gensym("vbap"), (t_newmethod)vbap_new,
    (t_method)vbap_free, (short)sizeof(t_vbap), 0, A_DEFFLOAT, A_DEFFLOAT,
    A_DEFFLOAT, 0);

  class_addbang(vbap_class, (t_method)vbap_bang);
  class_addmethod(vbap_class, (t_method)vbap_matrix,
    gensym("loudspeaker-matrices"), A_GIMME, 0);

  // define_loudspeaker messages
  class_addmethod(vbap_class, (t_method)vbap_def_ls,
    gensym("define-loudspeakers"), A_GIMME, 0);
  class_addmethod(vbap_class, (t_method)vbap_def_ls,
    gensym("define_loudspeakers"), A_GIMME, 0);
  class_addmethod(vbap_class, (t_method)def_ls_read_directions,
    gensym("ls-directions"), A_GIMME, 0);
  class_addmethod(vbap_class, (t_method)def_ls_read_triplets,
...
void main(void)
{
  setup((t_messlist **)&vbap_class, (method)vbap_new, 0L,
    (short)sizeof(t_vbap), 0L, A_DEFLONG,A_DEFLONG,A_DEFLONG, 0);
...
/* these are for getting data from a cold inlet on Max MSP,
   in Pd you use floatinlet_new() in new() */
/*----------------------------------------------------------------------------*/
...
//  return NULL;
  ...
  // refers to memory dynamically allocated when
  // a define_loudspeakers config is received
  x->x_ls_setCount = 0;
...
#define vbap_getmem_error(numbytes) \
  error("vbap: can't allocate an additional %ld bytes", numbytes);

// can currently allocate up to 256K to support up to 44 channels in 3D
// note: to save memory, the required amount for a given configuration could
//       instead be dynamically allocated by calling this method from the
//       vbap_matrix() method
static int vbap_getmem(t_vbap *x, int lsSetCount)
{
#ifdef PD
  ...
  if (!x->x_set_inv_matx)
  {
    vbap_getmem_error(sizeof(t_float *) * lsSetCount);
    return 0;
  }
  ...
  for (i = 0; i < lsSetCount; i++)
  {
    ...
    if (!x->x_set_inv_matx[i])
    {
      vbap_getmem_error(sizeof(t_float) * MATRIX_DIM);
      return 0;
    }
  }
  ...
  if (!x->x_lsset)
  {
    vbap_getmem_error(sizeof(long *) * lsSetCount);
    return 0;
  }
  ...
  for (i = 0; i < lsSetCount; i++)
  {
    ...
    if (!x->x_lsset[i])
    {
      vbap_getmem_error(sizeof(long) * SPEAKER_SET_DIM);
      return 0;
    }
  }

  unsigned long numbytes = 2 *
    (sizeof(t_float *) * lsSetCount * sizeof(t_float) * MATRIX_DIM) +
    (sizeof(long *) * lsSetCount * sizeof(long) * SPEAKER_SET_DIM);
  logpost(NULL, 3, "vbap: %ldKB allocated for instance", numbytes / 1000);
...
static void vbap_free(t_vbap *x)
{
  ...
  {
    freebytes(x->x_set_inv_matx[i], sizeof(t_float) * MATRIX_DIM);
...
static void spread_it(t_vbap *x, t_float *final_gs)
{
  ...
  if (x->x_dimension == 3)
  {
    ...
    vect_cross_prod(x->x_spread_base, vscartdir, spreadbase[1]);
...
static void vbap_matrix(t_vbap *x, Symbol *s, int ac, Atom *av)
{
  ...
  if (ac > 0)
  {
    ...
    if (d != 2 && d != 3)
    {
      pd_error(x, "vbap %s: dimension can be only 2 or 3", s->s_name);
      x->x_lsset_available = 0;
      return;
    }
  ...
  {
    pd_error(x, "vbap %s: bad empty parameter list", s->s_name);
    x->x_lsset_available = 0;
    return;
  }
  ...
  long counter = (ac - 2) /
    ((x->x_dimension * x->x_dimension * 2) + x->x_dimension);
  if (counter-1 > MAX_LS_SETS)
  {
    pd_error(x, "vbap %s: loudspeaker definitions exceed maximum number "
  ...
  if (counter == 0)
  {
    pd_error(x, "vbap %s: not enough parameters", s->s_name);
    x->x_lsset_available = 0;
    return;
  }
...
#ifdef PD
  logpost(NULL, 3, "vbap: loudspeaker setup configured");
#else // MAX
